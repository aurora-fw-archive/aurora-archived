...
namespace Aurora
{
    ...
    GUIWindow::GUIWindow(std::string name, int width, int height, int pos, arflags_t type)
    {
        ...
        Aurora::LastID++;
        ShellLog::Debug("creating new window: id_" + std::to_string(ID));
        ...
        connect("destroy", []{
            gtk_main_quit();
            ShellLog::Debug("window id_" + std::to_string(scid_tmp) + " is destroyed.");
        });
        show();
        ShellLog::Debug("window id_" + std::to_string(ID) + " is created.");
    }

    void GUIWindow::setTitle(std::string title) 
    {
        ShellLog::Debug("setting title on window id_" + std::to_string(ID));
        gtk_window_set_title(GTK_WINDOW(Window), title.c_str());
    }

    void GUIWindow::setPos(int pos)
    {
        ShellLog::Debug("setting window position on window id_" + std::to_string(ID));
    ...
    {
        ShellLog::Debug("creating new signal on window id_" + std::to_string(ID));
        g_signal_connect(Window, detailedSignal.c_str(), G_CALLBACK(signalFunction), signalData);
    }

    void GUIWindow::show(void)
    {
        ShellLog::Debug("show window id_" + std::to_string(ID));
    ...
    {
        (*startFunction)();
        ShellLog::Debug("starting main loop on window id_" + std::to_string(ID));
        gtk_main();
    }
}